...
|----------|-----------|----------|
| Разброс `ln r_AC` по матчам A–C | σ(ln) > **0.25** или MAD велик | Ненадёжно |
| Разброс `ln r_BC` по матчам B–C | σ(ln) > **0.25** или MAD велик | Ненадёжно |
| Конфликт перевертышей | Два правила на один матч с несовместимым знаком | Ненадёжно |
| Нет валидных odds | После валидации 0 матчей в цепочке | Ненадёжно |
| Итоговый reliability_score | < **0.5** | Ненадёжно |

**Разброс мерим в лог-пространстве.** Так как `r_AC` — отношение (лог-нормальное, тяжелохвостое), вместо CV (`σ/μ`, сам чувствителен к выбросам) используем **стандартное отклонение `ln r`** или **MAD** (median absolute deviation). Это устойчивее, особенно при больших `n`. Порог MVP по `σ(ln r)`: **0.25**.
...
score = w1 * min(1, n_AC / 3) * min(1, n_BC / 3)
      + w2 * freshness_factor
      + w3 * (1 - min(1, D_AC / 0.25)) * (1 - min(1, D_BC / 0.25))
```

Где `n_AC`, `n_BC` — число валидных closing-матчей; `freshness_factor` — средняя свежесть (1 − возраст/max_odds_age); `D = σ(ln r)` (или MAD) — разброс в лог-пространстве; веса MVP: `w1=0.4`, `w2=0.3`, `w3=0.3`.
