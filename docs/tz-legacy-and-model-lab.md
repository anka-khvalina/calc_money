...
UI лаборатории при открытии вкладки **обязан** подставить все дефолты из этих файлов (режим отчёта, маржа, флаги, коэффициенты Legacy/Auto). Под каждым контролом — hint «зачем настраивается».
...
### 2.1. UI: подписи и дефолты из конфига

**Каждая настройка** на вкладке (режим, галки «Считать», фильтры, маржа, параметры моделей, переключатели компонентов) обязана иметь **подпись снизу** — короткое пояснение «зачем и для чего».

Паттерн как на вкладке «Линия»:

```text
[ контрол ]
подпись-hint мелким текстом
```

**Все числовые/булевы коэффициенты моделей** (α/γ/q/rho-кандидаты, калибровки, train-веса, маржа по умолчанию и т.д.) берутся из `model_config.json` и **подставляются по умолчанию** при открытии вкладки — как раньше на «Линии». Хардкод в HTML/JS только как fallback, если ключ в конфиге отсутствует.

При смене значений в UI (если редактирование разрешено) источником истины для следующего прогона остаётся текущее состояние контролов; сброс «по умолчанию» = перечитать конфиг.
...
| R-AC10 | У каждой настройки есть подпись-hint снизу |
| R-AC11 | Дефолты коэффициентов/маржи/режима читаются из `model_config.json` |
