...
Подгонка по Shin-вероятностям 1X2. Опционально **Dixon–Coles** (γ).

**Контроль стабильности:** ожидается `a≈0`, `b≈1`, `c≈0`, `d≈1`. Сильные отклонения (напр. `b=0.45`, `d=1.8`, `c=−0.9`) — сигнал, что база S/D слабая или мало данных. В UI: **«Калибровка 1X2: стабильна / нестабильна»**.

### Шаг 7. Модель ничьи (если включена)

Обучение logit(P_X) от |D| и S на истории.  
В **прогнозе** целевая ничья слегка корректирует диагональ матрицы (после DC):
...
- тревога: **q clamp rate > 12%** — q часто упирается в q_min/q_max → базовая матрица+DC не согласны с draw model
