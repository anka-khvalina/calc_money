...
**Контроль стабильности:** ожидается `a≈0`, `b≈1`, `c≈0`, `d≈1`. Сильные отклонения — сигнал, что база S/D слабая или мало данных.

### Шаг 6b. Auto 1X2 калибровка (только Auto)

После подбора `α` / `ρ` программа **перекалибрует** `a,b` (и при необходимости `c,d`) уже на итоговой joint matrix (`α,ρ`), без DC и без draw-model. Затем может уточнить `α/ρ` с приоритетом рынков **1X2 > AH > OU**.
