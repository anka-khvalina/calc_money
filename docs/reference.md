...
На вкладке «История» также есть массовые операции по выбранному сезону (лиге):
- **активность** — даты + включить/отключить → PATCH `matches.active`;
- **вес матчей** — даты + `match_weight` (default `1`, ≥ 0) → PATCH `matches.match_weight`.
  Рядом — справка «Как разделить сезон и какие веса поставить» с расчётом периодов по датам выбранного сезона
  (рекомендация по умолчанию: `1.0 / 0.8 / 0.6` для периодов 0–60 / 61–120 / >120 дней).
