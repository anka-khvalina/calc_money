...
Ядро без обучения и GUI (импортируемо для тестов):
  ...
  * Пуассон-матрица и рынки из матрицы (1X2, AH, OU, ИТ, exact);
  * восстановление скрытых S_m / D_m из closing-линий;
  * legacy helpers: Dixon-Coles / draw adjust (для old-vs-new compare).

Актуальный web-flow: Auto Marginals + Gaussian Copula — см. `goal_matrix_auto.py`
и runtime-конфиг `config/goal_matrix.json` (без изменений БД).
"""
